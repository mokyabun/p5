...
export function sketch(p: p5) {
	...
	p.draw = () => {
		...
		for (let art of artifacts) {
			...
			const posSpeed = 0.0007;
			art.xOffset += posSpeed;
			art.yOffset += posSpeed;
			art.angleOffset += posSpeed * 2;
			art.alphaOffset += 0.02;
	...
	function drawCircleCluster(art: any) {
		...
		for (let i = 0; i < numCircles; i++) {
			...
			const colorProgress = (art.colorIndex + p.frameCount * 0.005) % palette.length;
	...
	function drawGeometricCircles() {
		...
		const pulse = p.sin(p.frameCount * pulseSpeed);
	...
	function addNoise() {
		...
		for (let i = 0; i < p.pixels.length; i += rgbaStep) {
			let noiseAmount = p.random(-noiseIntensity, noiseIntensity);
